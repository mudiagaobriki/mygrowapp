...
function UserController(){
    const register  = async (req, res) => {
        try {
            // Get user input
            const {firstName, lastName, email, password, phone, type='customer', country='NG', countryCode='234'} = req.body;
            ...
            sendEmail(email, "", "Grow App", "Grow App <noreply@mygrowapp.com>",
    ...
    const login = async (req, res) => {
        try {
            ...
            if (!user) {
                return res.status(400).send("Email not yet registered. Please sign up to create an account");
            }
    ...
    const resendVerificationLink = async (req, res) => {
        ...
        if (user) {
            ...
            // send verification email
            sendEmail(email, "", "Betweysure", "Betweysure <noreply@betweysure.com>",
                "Registration Successful", "", template(templateData))
    ...
    const forgotPassword = async (req, res) => {
        ...
        if (user) {
            ...
            // create email verification link
            const secret = jwt.sign({email}, process.env.JWT_SECRET)
            const verifyUrl = `/email/reset/${new Buffer.from(secret, 'utf8').toString('base64')}`;
            const verificationUrl = `${process.env.FRONT_END}${verifyUrl}`;
    ...
    return { register, login, verifyUser, resendVerificationLink, forgotPassword, resetPassword }
}
